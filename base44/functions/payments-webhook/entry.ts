...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant: activate the subscription plan on the user's record.
  // Signed-in buyer → update by appUserId. Anonymous buyer → match by email
  // (they registered before paying, so a User row should exist).
  if (purchase.appUserId) {
    await db.entities.User.update(purchase.appUserId, {
    ...
    if (users?.[0]) {
      await db.entities.User.update(users[0].id, {
        plan: purchase.productId,
        subscription_status: "active",
      });
    }
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke: mark the user's subscription as canceled (access expires with the billing cycle).
  if (purchase.appUserId) {
    await db.entities.User.update(purchase.appUserId, { subscription_status: "canceled" });
  } else if (purchase.buyerEmail) {
    const users = await db.entities.User.filter({ email: purchase.buyerEmail });
    if (users?.[0]) {
      await db.entities.User.update(users[0].id, { subscription_status: "canceled" });
    }
  }
  // ===== END APP-SPECIFIC =====
