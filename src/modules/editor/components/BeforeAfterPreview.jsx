...
import { adjustmentsToCssFilter } from "../utils/exposureEngine.js";
...
export default function BeforeAfterPreview({ photo, adjustments, showBefore, onToggle }) {
  return (
    <div className="bg-card rounded-2xl border border-border overflow-hidden relative">
      {photo ? (
        <>
          <Image src={photo.file_url} className="w-full aspect-[4/3]" fittingType="fit" style={showBefore ? {} : { filter: adjustmentsToCssFilter(adjustments) }} />
