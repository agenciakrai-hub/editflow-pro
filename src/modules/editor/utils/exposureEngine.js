...
// CSS filter approximation for the before/after preview.
export function adjustmentsToCssFilter(a = {}) {
  const brightness = 1 + (a.exposure ?? 0) / 200;
  const contrast = 1 + (a.contrast ?? 0) / 200;
  const saturate = 1 + (a.vibrance ?? 0) / 150;
  const sepia = Math.max(0, (a.temperature ?? 0)) / 400;
  const hue = (a.tint ?? 0) / 8;
