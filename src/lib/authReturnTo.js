...
// Resolve ?returnTo= to a safe same-origin path, else "/proyectos".
//
...
export function safeReturnTo() {
  ...
  if (!raw) return "/proyectos";
  try {
    const url = new URL(raw, window.location.origin);
    if (url.origin !== window.location.origin) return "/proyectos";
    ...
    if (!path.startsWith("/") || path.startsWith("//") || path.includes("\\")) return "/proyectos";
    return path;
  } catch {
    return "/dashboard";
  }
